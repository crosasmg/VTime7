Option Strict Off
Option Explicit On
Public Class Durinsu_prods
	Implements System.Collections.IEnumerable
	'%-------------------------------------------------------%'
	'% $Workfile:: Durinsu_prods.cls                        $%'
	'% $Author:: Nvaplat7                                   $%'
	'% $Date:: 9/08/03 12:35p                               $%'
	'% $Revision:: 8                                        $%'
	'%-------------------------------------------------------%'
	...
	'**% Add: Add a new instance of the class Durinsu_prod to the collection
	'% Add: Añade una nueva instancia de la clase Durinsu_prod a la colección
	Public Function Add(ByRef objClass As Durinsu_prod) As Durinsu_prod
		If objClass Is Nothing Then
			objClass = New Durinsu_prod
		End If
		
		With objClass
			mCol.Add(objClass, "DIP" & .nBranch & .nProduct & .nIdurafix & .nTypdurins & .dEffecdate.ToString("yyyyMMdd"))
		End With
		Add = objClass
	End Function
	...
	Default Public ReadOnly Property Item(ByVal vntIndexKey As Object) As Durinsu_prod
		Get
	...
	Public Function Find(ByVal nBranch As Integer, ByVal nProduct As Integer, ByVal dEffecdate As Date, Optional ByVal lblnFind As Boolean = False) As Boolean
		...
		If mlngBranch <> nBranch Or mlngProduct <> nProduct Or mdtmEffecdate <> dEffecdate Or lblnFind Then
			...
			'+Definición de parámetros para stored procedure 'ReaDurinsu_prod_a'
			With lrecReaDurinsu_prod
				.StoredProcedure = "ReaDurinsu_prod_a"
				...
				If .Run Then
					Do While Not .EOF
						...
						lclsDurinsu_prod.nBranch = nBranch
						lclsDurinsu_prod.nProduct = nProduct
						lclsDurinsu_prod.nIdurafix = .FieldToClass("nIdurafix")
						lclsDurinsu_prod.dEffecdate = .FieldToClass("dEffecdate")
						lclsDurinsu_prod.nTypdurins = .FieldToClass("ntypdurins")
						lclsDurinsu_prod.nMinDurIns = .FieldToClass("nMinDurIns")
						Call Add(lclsDurinsu_prod)
						.RNext()
