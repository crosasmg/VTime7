...
//+ Variable para el control de versiones
    document.VssVersion="$$Revision: 16 $|$$Date: 18/11/04 10:44 $"        
...
//-----------------------------------------------------------------------------------
    with(self.document.forms[0]){
        ...
        if(tcnLoanAmount.value>0 || insConvertNumber(tcnCommBase.value)>0 ){
            if(lintCurrency != cbeCurrency.value){
				...
				if (tcnPolicy.value != 0 && tcnPolicy.value != ''){
    				strParams = "nBranch=" + cbeBranch.value + 
								...
								"&Date=" + tcdEffecDate.value + 
 								"&nPercent=" + tcnPercent.value +
 								"&nLoanType=" + cbeLoanType.value; 
...
//					tcnLoanAmount.value = VTFormat(insConvertNumber(tcnCommBase.value) * insConvertNumber(Field.value)/100, '', '', '', 6, true);				
					nValue = insConvertNumber(tcnCommBase.value) * insConvertNumber(Field.value)/100;
					if (cbeCurrency.value == 1){
                        nValue = Math.round(nValue);					    
					}
					tcnLoanAmount.value = VTFormat(nValue, '', '', '', 6, true);
...
'Boleta
If mobjIntermedia.nLegal_sch = 1 Then
	Response.Write(mobjValues.HiddenControl("hddTypesupport", "3"))
	'Solo si es Rentas Vitalicias Previsionales y el esquema legal es Factura    
ElseIf mobjIntermedia.nLegal_sch = 2 And mobjProduct.nProdClas = 9 Then 
	Response.Write(mobjValues.HiddenControl("hddTypesupport", "1")) 'TIPO DE DOCUMENTO A LA ORDEN DE PAGO
Else
