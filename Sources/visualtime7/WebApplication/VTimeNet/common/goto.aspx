...
<script language="VB" runat="Server">
    ...
    Private Sub insMakeURL(ByRef sCodisp As String, ByRef lintWindowty As Object, ByRef lstrModule As String, ByRef lstrProject As String, ByRef lintHeight As Object, ByRef sCodispl As String, ByRef sQueryString As String)
        ...
        Dim lintUsercode As Integer
        Dim lintScodisplPrev As String
        Dim lintGoTo As Integer

        If Request.QueryString("ActionByToolbar") = 396 Then
            lintUsercode = Session("nUsercode")
            lintScodisplPrev = Trim(Mid(Session("sHistory"), 1, 8))
        End If

        lstrHref = "/VTimeNet/Common/" & insGetBaseName(lintWindowty) & "?sCodispl=" & sCodispl & "&sModule=" & lstrModule & "&sProject=" & lstrProject & "&nHeight=" & lintHeight & "&sCodisp=" & sCodisp & sQueryString & "&nUsercode=" & lintUsercode & "&prevScodispl=" & lintScodisplPrev
    ...
    Private Function insNavigateTo() As Object
        ...
        With lobjQuery
            If .OpenQuery("windows", "nWindowty, nModules, sDescript, nHeight,sCodisp", "sCodispl='" & lstrCodispl & "'") Then
                ...
                If lintHeight = 0 Or lintHeight = eRemoteDB.Constants.intNull Then
                    lintHeight = 130
                Else
...
//+ en otra ventana
			if(typeof(opener.top.frames["FraModules"])!='undefined'){
				win = open('/VTimeNet/Common/GoTo.aspx?sCodispl=' + sCodispl, 'Transaccion','toolbar=no,resizable=no,location=no,directories=no,status=yes,menubar=no,copyhistory=no,width=4000,height=4000,left=20,top=20');
				win.moveTo(0, 0);
