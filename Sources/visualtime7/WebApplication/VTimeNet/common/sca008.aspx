...
Private Sub SafeEndorsements()
	...
	If CStr(Session("sFirst")) <> "No" Then
		Session("sFirst") = "No"
		Call lobjEndorsLetters.FindEndorsLetters(Session("sClient"), Session("nUsercode"), Request.QueryString.Item("sCodispl"), Session("sCertype"), Session("nBranch"), Session("nProduct"), Session("nPolicy"), Session("nCertif"))
	End If
...
Response.Write("</" & "SCRIPT>")
	...
	'UPGRADE_WARNING: Date was upgraded to Today and has a new behavior. Copy this link in your browser for more: 'ms-help://MS.VSCC.2003/commoner/redir/redirect.htm?keyword="vbup1041"'
        If lcolRequests.Find(Left(Request.QueryString.Item("sCodispl"), 6), _
                                insGetAction(), _
                                getVariable("scertype"), _
                                mobjValues.StringToType(getVariable("nbranch"), eFunctions.Values.eTypeData.etdLong), _
                                mobjValues.StringToType(getVariable("nproduct"), eFunctions.Values.eTypeData.etdLong), _
                                ...
                                mobjValues.StringToType(getVariable("ncase_num"), eFunctions.Values.eTypeData.etdLong), _
...
<%If Request.QueryString.Item("Type") = "PopUp" Then%>
          lstrQueryString = "/VTimeNet/Letter/Letter/Variables.aspx?sCustomLetter=1&sCodispl=<%=Request.QueryString.Item("sCodispl")%>&Type=upd&Action=" + lstrAction + "&Location=" + lstrLocation + "&nLetterNum=" + document.forms[0].tcnLetterNum.value + "&nLettRequest=" + document.forms[0].tcnLettRequest.value + "&nLanguage=" + document.forms[0].cbeLanguage.value + "&sBtn=" + lintbtn;
<%Else%>
          lstrQueryString = "/VTimeNet/Letter/Letter/Variables.aspx?sCustomLetter=1&sCodispl=<%=Request.QueryString.Item("sCodispl")%>&Type=Qry&Action=" + lstrAction + "&Location=" + lstrLocation +  "&nLetterNum=" + marrArray[lintIndex].tcnLetterNum + "&nLettRequest=" + marrArray[lintIndex].tcnLettRequest + "&nLanguage=" + marrArray[lintIndex].tcnLanguage  + "&sBtn=" + lintbtn;
<%End If%>        
...
<% If Left(Request.QueryString.Item("sCodispl"), 5) <> "SCA80" Then %>
	<TABLE WIDTH=8% align = right border = 0>
	 <TR>
	 <TD width=40% ></TD>
		<TD width=5% ><%=mobjValues.AnimatedButtonControl("btnHelp", "/VTimeNet/images/A601Off.png", mobjValues.getMessage(10408),  , "insHelp();")%>
	    </TD>
	    <TD width=5% ><%=mobjValues.AnimatedButtonControl("btnAbout", "/VTimeNet/images/A603Off.gif", mobjValues.getMessage(10407),  , "insAbout('SCA805');")%>
	    </TD>
	 <TD width=40% ></TD>
	</TABLE><BR><BR>
<% End If %> 
