...
Private Sub insDefineHeader()
	...
	With mobjGrid.Columns
		...
		.AddPossiblesColumn(0, GetLocalResourceObject("cbeTyp_ProfitworkerColumnCaption"), "cbeTyp_Profitworker", "Table950", eFunctions.Values.eValuesType.clngComboType,  ,  ,  ,  ,  , "InsChangenTypeProfitWorker(this);", True, True, GetLocalResourceObject("cbeTyp_ProfitworkerColumnToolTip"))
		mobjGrid.Columns("cbeTyp_Profitworker").TypeList = 1
		mobjGrid.Columns("cbeTyp_Profitworker").List = "1,2"
...
'% insPreVI016: Esta función se encarga de cargar los datos en la forma "Detalle" 
'--------------------------------------------------------------------------------------------
Private Sub insPreVI016()
	'--------------------------------------------------------------------------------------------
	Dim nAvailToBuy As Object
	Dim ldblBalance As Object
	Dim lintCount As Object
	
	Dim nQuotavalue As Double
	
	With mobjValues
		If mcolFunds_pols.Find_VI016("2", .StringToType(Request.QueryString.Item("nBranch"), eFunctions.Values.eTypeData.etdDouble), .StringToType(Request.QueryString.Item("nProduct"), eFunctions.Values.eTypeData.etdDouble), .StringToType(Request.QueryString.Item("nPolicy"), eFunctions.Values.eTypeData.etdDouble), .StringToType(Request.QueryString.Item("nCertif"), eFunctions.Values.eTypeData.etdDouble), .StringToType(Request.QueryString.Item("dEffecdate"), eFunctions.Values.eTypeData.etdDate), .StringToType(Request.QueryString.Item("nOrigin"), eFunctions.Values.eTypeData.etdDouble)) Then
			...
			' nAvailToBuy = mclsFunds_Pol.insCalAvailable("2",                                                         '.StringToType(Request.QueryString("nBranch"),eFunctions.Values.eTypeData.etdDouble),                                                         '.StringToType(Request.QueryString("nProduct"),eFunctions.Values.eTypeData.etdDouble),                                                         '.StringToType(Request.QueryString("nPolicy"),eFunctions.Values.eTypeData.etdDouble),                                                         '.StringToType(Request.QueryString("nCertif"),eFunctions.Values.eTypeData.etdDouble),                                                         '.StringToType(Request.QueryString("dEffecdate"),eFunctions.Values.eTypeData.etdDate),                                                         '.StringToType(Request.QueryString("nOrigin"), eFunctions.Values.eTypeData.etdDouble))
			lintCount = 0
			For	Each mclsFunds_Pol In mcolFunds_pols
				With mobjGrid
					...
					.Columns("hddstyp_rat").DefValue = CStr(mclsFunds_Pol.nTyp_Profitworker)
					.Columns("cbeTyp_Profitworker").DefValue = CStr(mclsFunds_Pol.nTyp_Profitworker)
					.Columns("cbeTyp_Profitworker").Descript = mclsFunds_Pol.sShort_Des
					.Columns("tcnUnits").DefValue = CStr(mclsFunds_Pol.nQuan_avail)
					.Columns("tcnTotal_Amount").DefValue = CStr(mclsFunds_Pol.nAmount)
					nQuotavalue = nQuotavalue + mclsFunds_Pol.nAmount
					...
					.Columns("btnSignal").src = "/VTimeNet/Images/btnLargeAddOff.png"
					.Columns("tcnSignal").DefValue = CStr(1)
					.Columns("tcnAvailable").DefValue = CStr(mclsFunds_Pol.nAvailToBuy)
					...
					End If
					If CDbl(mclsFunds_Pol.sSel) = 3 Then
					...
					If CDbl(mclsFunds_Pol.sSel) > 2 Then
						mclsFunds_Pol.sSel = "1"
					End If
...
'% insPreVI016Upd: Se define esta funcion para contruir el contenido de la ventana UPD de los cambios de fodos de inversiones
'----------------------------------------------------------------------------------------------------------------------------
Private Sub insPreVI016Upd()
	'----------------------------------------------------------------------------------------------------------------------------
	With Request
		Response.Write(mobjGrid.DoFormUpd(.QueryString.Item("Action"), "valPolicyTra.aspx", "VI016", .QueryString.Item("nMainAction"), mobjValues.ActionQuery, CShort(.QueryString.Item("Index"))))
	End With
     ...
     if (sTypeWindow == 'PopUp') {
        try {
		    with (self.document.images["btnSignal"]){
	    		...
	    		if ((document.forms[0].tcnSignal.value==1?2:1) == 2)
	    			{ document.forms[0].cbeTyp_Profitworker.disabled = false; }
	    		else { document.forms[0].cbeTyp_Profitworker.disabled = true; }
		    }
	    } 
	    catch(error){}
    }
...
//-------------------------------------------------------------------------------------------
function InsChangenTypeProfitWorker(sField){
//-------------------------------------------------------------------------------------------
    var lstrParams;
    with (self.document.forms[0])
		{
		insDefValues('InsChangeTyp_Profitworker','nType='       + sField.value + 
									             ...
									             '&nOrigin='    + hddnOrigin.value,'/VTimeNet/policy/policytra');
		}
	}
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
with(self.document.forms[0]){
	if (hddstyp_rat.value == '') {
      cbeTyp_Profitworker.disabled = false;
      tcnAvailable.value = VTFormat('0', '', '', '', 6, false);
       }
   else {
      cbeTyp_Profitworker.disabled=true; }
    }
}
function Confirm() {
    var lstrParams; 
...
<FORM METHOD="post" ID="FORM" NAME="VI016" ACTION="valPolicyTra.aspx?x=1">
<%
Response.Write("<SCRIPT>var sTypeWindow='" & Request.QueryString.Item("Type") & "'</SCRIPT>")
If Request.QueryString.Item("Type") <> "PopUp" Then
	With Response
		.Write(mobjMenu.setZone(2, "VI016", Request.QueryString.Item("sWindowDescript"), CShort(Request.QueryString.Item("nWindowTy"))))
		.Write(mobjValues.ShowWindowsName("VI016", Request.QueryString.Item("sWindowDescript")))
...
Response.Write(mobjValues.HiddenControl("hddnOrigin", Request.QueryString.Item("nOrigin")))
...
If Request.QueryString.Item("sProcessType") = "2" Then
	...
	Else
		...
		Response.Write("<SCRIPT>insChangeSignal();</SCRIPT>")
	End If
Else
