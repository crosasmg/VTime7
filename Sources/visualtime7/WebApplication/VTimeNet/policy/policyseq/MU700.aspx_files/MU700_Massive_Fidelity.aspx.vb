...
Public Class MU700_Massive_Fidelity_aspx
    ...
    Dim lstrErrors As String = ""
            Protected Overrides Sub OnLoad(ByVal e As System.EventArgs)
                ...
                If Request.QueryString("MassiveFidelity") = "1" Then
                    If  MassiveFidelity() Then
    ...
    Private Function MassiveFidelity() As Boolean
    ...
    Public Function  FileToArray(ByVal lstrFile As String) As Boolean
        ...
        If Len(Dir(lstrFiledeltxt, FileAttribute.Archive)) > 0 And lstrFiledeltxt <> String.Empty Then
            ...
            If Not EOF(lintFileNum) Then
                listSbFileText.Add(sbFileinText)
            End If
            ...
            Do While Not EOF(lintFileNum) And lblnContinue
                ...
                Else
                    ...
                    'Response.Write("longitud del bloque-> " & (sbFileinText.Length + lstrArray_txt.Length).ToString & vbCrLf)
                    If (sbFileinText.Length + lstrArray_txt.Length) > 3500 Then
                        sbFileinText = New StringBuilder()
                        listSbFileText.Add(sbFileinText)
                    End If
                    sbFileinText.Append(String.Join(";", lstrArray_txt) & "|")
                    'Response.Write("Contruccion fila ->" & String.Join(";", lstrArray_txt) & "|" & vbCrLf)
                    lintColumn = 0
                    On Error Resume Next
        ...
        On Error GoTo ja
        'Response.Write("Cantidad de bloques->" & listSbFileText.Count & vbCrLf)
        'Dim indx As Integer
        Dim result As Boolean
        For Each sbFileinTextItem In listSbFileText
            'indx += 1
            'Response.Write("Bloque N°->" & indx.ToString() & vbCrLf)
            'Response.Write(" Data: -> " & sbFileinTextItem.ToString() & vbCrLf)
            If sbFileinTextItem.Length > 0 Then
                lstrErrors = (New ePolicy.ValPolicySeq_MU700).ValMassiveCharge_MultiRisk_Det(sbFileinTextItem.ToString(), 6)
                'Response.Write(" ResultValidacion: -> " & lstrErrors & vbCrLf)
                If lstrErrors Is Nothing Then
                    ...
                    With lrecMassiveCharge_MultiRisk_Det
                        .StoredProcedure = "insMassiveCharge_Fidelity"
                        ...
                        .Parameters.Add("sclient_fidelity", Session("sClient"), eRemoteDB.Parameter.eRmtDataDir.rdbParamInput, eRemoteDB.Parameter.eRmtDataType.rdbDouble, 22, 0, 10, eRemoteDB.Parameter.eRmtDataAttrib.rdbParamNullable)
                        ...
                        .Parameters.Add("sFileinText", sbFileinTextItem.ToString(), eRemoteDB.Parameter.eRmtDataDir.rdbParamInput, eRemoteDB.Parameter.eRmtDataType.rdbVarchar, 4000, 0, 0, eRemoteDB.Parameter.eRmtDataAttrib.rdbParamNullable)
                        .Run(False)
                    End With
                    result = True
                Else
...
ja:
	...
	Public Function insTransformationExcel(ByVal sFile As String) As Boolean
		...
		mvarSalidaExcel.ActiveWorkbook.SaveAs(lstrFileName & Trim(lstrFile) & ".TXT", Microsoft.Office.Interop.Excel.XlPivotFieldDataType.xlText, False,,,,,,,,,False)
        sMessage &= "9_"
