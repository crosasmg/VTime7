...
//                    document.forms[0].tcnPolicy.disabled = true;        
//                }
		insDefValues("dLedgerdat","nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&dEffecdate=" + tcdLedgerDate.value + "&nTransaction=" + cbeTransactio.value,'/VTimeNet/Policy/PolicySeq');                
		insDefValues(sField,"nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&dEffecdate=" + tcdEffecdate.value + "&nTransaction=" + cbeTransactio.value,'/VTimeNet/Policy/PolicySeq');                
                break;
            case "Certificat":
                if(tcnCertificat.value!="")
                    if (cbeBranch.value!="0" &&
                        valProduct.value!="" &&
                        tcnPolicy.value!="")
                        insDefValues(sField, "sCertype=" + sCertype.value + "&nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&nPolicy=" + tcnPolicy.value  + "&nCertif=" + tcnCertificat.value + "&nTransaction=" + cbeTransactio.value + "&nQuotProp=" + tcnQuotProp.value,'/VTimeNet/Policy/PolicySeq')
                break; 
            case "Policy":
//                if (cbeBranch.value!="0" &&
//                    valProduct.value!="" &&
//                    tcnPolicy.value!="")
                    insDefValues(sField, "sCertype=" + lstrCertype + "&nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&nPolicy=" + tcnPolicy.value + "&nCertif=" + tcnCertificat.value + "&nTransaction=" + cbeTransactio.value + "&nQuotProp=" + tcnQuotProp.value + "&dEffecdate=" + tcdEffecdate.value,'/VTimeNet/Policy/PolicySeq')
//                    if (self.document.forms[0].cbeTransactio.value == 45)
            ...
            case "Agency":
                ...
                break;
            case "nServ_order":
                insDefValues(sField, "sCertype=" + sCertype.value + "&nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&nPolicy=" + tcnPolicy.value  + "&nCertif=" + tcnCertificat.value + "&nTransaction=" + cbeTransactio.value + "&dEffecdate=" + tcdEffecdate.value + "&nType_amend=" + valType_amend.value + "&nServ_order=" + tcnServ_order.value,'/VTimeNet/Policy/PolicySeq')
                break;
            case "TabCertif":
                lstrClient = InsValuesCero(valCertif);
                
                valCertif.value = lstrClient;
                
                insDefValues(sField, "sCertype=" + sCertype.value + "&nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&nPolicy=" + tcnPolicy.value  + "&sClient=" + valCertif.value + "&dEffecdate=" + tcdEffecdate.value,'/VTimeNet/Policy/PolicySeq')
                break;
        }
    }
...
//---------------------------------------------------------------
    with (self.document.forms["CA001"]) {
        ...
        if (mstrTransaction == 6 ||
            ...
            tcnPolicy.value = "";
            tcnCertificat.value = "";
            insDefValues("Propolcer", "sCertype=" + sCertype.value + "&nBranch=" + cbeBranch.value + "&nProduct=" + valProduct.value + "&nPolicy=" + tcnPolicy.value  + "&nCertif=" + tcnCertificat.value + "&nTransaction=" + cbeTransactio.value,'/VTimeNet/Policy/PolicySeq')
        }
        else{
            insSelTransaction();
...
//+ Si la función se invoca cuando se carga la página por primera vez...    
    ...
    with (self.document.forms["CA001"]) {
        ...
        ShowDiv('divFolio', 'hide');
        ShowDiv('divFolio2', 'hide');
...
//+ Póliza destino            
            ...
            ShowDiv('divPol_dest3', 'hide');
            ShowDiv('divPol_dest4', 'hide');
        }
    }    
...
//--------------------------------------------------------------------------------------------   
    if ((ntransactio != ePolTransac.clngTransHolder)&&
        (ntransactio != ePolTransac.clngDuplPolicy)){
        ShowDiv('divPol_dest', 'hide');
        ShowDiv('divPol_dest2', 'hide');
        ShowDiv('divPol_dest3', 'hide');
        ShowDiv('divPol_dest4', 'hide');
        ...
        if (ntransactio == ePolTransac.clngTransHolder){
            ShowDiv('divPol_dest3', 'show');
            ShowDiv('divPol_dest4', 'show');
        }
    }
}	
...
//+ Se inhabilitan los campos ramo y producto        
    ...
    switch(mstrTransaction){
        ...
        case ePolTransac.clngQuotPropAmendentConvertion:
        case ePolTransac.clngQuotPropRenewalConvertion:
            UpdateDiv('divCotProp', mstrPolicyDescript4);
            break;
        case ePolTransac.clngPolicyPropAmendent:
        case ePolTransac.clngCertifPropAmendent:
        case ePolTransac.clngPolicyPropRenewal:
        ...
        case ePolTransac.clngPropRenewalQuery:
            UpdateDiv('divCotProp', mstrPolicyDescript6);
            break;
        case "43":
        case "44":
            UpdateDiv('divCotProp', mstrPolicyDescript43);
            break;
        case ePolTransac.clngPropQuotConvertion:
            UpdateDiv('divCotProp', mstrPolicyDescript6);
            break;
        case ePolTransac.clngProposalConvertion:
            UpdateDiv('divCotProp', mstrPolicyDescript1);
            break;
        default:
            UpdateDiv('divCotProp', mstrPolicyDescript4 + "/" + mstrPolicyDescript6);
    }
...
//+ Cotización
        case ePolTransac.clngPolicyQuotation:
            ...
            document.forms["CA001"].tcnPolicy.disabled = false;
            document.forms["CA001"].btntcnPolicy.disabled = false;
            document.forms["CA001"].tcdEffecdate.value = GetDateSystem();
            mstrCertype = "3"
            lstrPolicyDescript = mstrPolicyDescript4 //"<%= mclsPolicy.TransactionCA001(4,True)%>"
            document.forms["CA001"].tcnProp_Reg.disabled = true;
            break;            
...
//+ El campo Cotización/Propuesta se habilita sólo si se trata de cotizaciones/propuestas de modificación/renovación
    if(mstrTransaction==ePolTransac.clngPolicyQuotAmendent ||
       ...
       mstrTransaction=="44") {
        ...
        with(document.forms["CA001"]){
            if (mstrTransaction == ePolTransac.clngPropQuotConvertion || mstrTransaction == ePolTransac.clngProposalConvertion) {
                tcnQuotProp.disabled = false
            } else {
...
//+ Se muestra el frame Póliza destino sólo en caso de Traspaso de asegurado
    if(mstrTransaction==ePolTransac.clngTransHolder ||
       mstrTransaction==ePolTransac.clngDuplPolicy){
        ShowDiv('divPol_dest', 'show')
        ShowDiv('divPol_dest2', 'show')
