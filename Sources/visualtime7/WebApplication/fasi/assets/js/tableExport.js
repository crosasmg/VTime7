    ...
    $.fn.tableExport = function (options) {
        ...
        if (defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt') {
            ...
            if (defaults.pdfmake.enabled === true) {
                ...
                var rData = [];
                var rep = 1;
                var split = defaults.jspdf.split;
                var detail = false;
                $hrows = collectHeadRows($(this));
                $rows = collectRows($(this));
                if ($hrows[0].cells[0].className == 'detail'){
                    detail = true;
                }
                $($hrows).each(function () {
                    ForEachVisibleCell(this, 'th,td', rowIndex, length,
                        function (cell, row, col) {
                            if (cell.className == 'detail')
                                return;
                            if (typeof cell !== 'undefined' && cell !== null) {
                                var colspan = getColspan(cell);
                                var rowspan = getRowspan(cell);
                                var cellValue = parseString(cell, row, col) || ' ';
                                hColumns.push({ colSpan: colspan, rowSpan: rowspan, content: replaceAccent(cellValue) });
                            } else
                            hColumns.push(' ');
                        });
                });
                    ...
                    ForEachVisibleCell(this, 'th,td', rowIndex, length,
                        function (cell, row, col) {
                            if (typeof cell !== 'undefined' && cell !== null) {
                                ...
                                var rowspan = getRowspan(cell);
                                var cellValue = parseString(cell, row, col) || ' ';
                                rData.push({ colSpan: colspan, rowSpan: rowspan, content: replaceAccent(cellValue) });
                            } else
                            rData.push(' ');
                        });
                });
                if (detail)
                    rData = rData.slice(2);
                if(hColumns.length > split){
                    rep = parseInt(hColumns.length / split);
                    if((hColumns.length % split) > 0){
                        rep++;
                    }
                }
                for (var i = 0; i < rep; i++) {
                    var currentColumn = hColumns.slice((i * split),  ((i * split) + split))
                    var currentData=[];
                    for (var y = 0; y < rData.length; y+=hColumns.length) {
                        currentData.push(rData.slice((i * split) + y,  ((i * split) + y + split)));
                    }
                    teOptions.doc.autoTable(currentColumn, currentData, {
                        startY: i === 0 ? 10 : teOptions.doc.lastAutoTable.finalY + 100,
                        pageBreak: 'auto',
                        margin: defaults.jspdf.margins,
                        styles: {
                            fontSize: defaults.jspdf.autotable.styles.fontSize,
                            overflow: defaults.jspdf.autotable.styles.overflow
                        }
                    });
                }          
        ...
        function replaceAccent (str) {
            var accent = [ 'Á','Â','á','É','é','Í', 'í', 'Ó', 'ó', 'Ú', 'ú', 'Ñ', 'ñ' ];
            var noaccent = ['A','A','a','E','e','I','i','O','o','U','u','N','n'];
                ...
                for (_C = C = 0; _C < elts.length; ++_C) {
                    ...
                    if (v != null) {
                        ...
                        if (v.length === 0)
                            ...
                            //NS: Se agregar este codigo de forma qe la fecha sean exportado como texto y asi vitar cualquier comportamento regional.
                            o.t = 's';
                            o.v = v;
                            o.z = defaults.date.html.toLowerCase();
                        }
                    }
