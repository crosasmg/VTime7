...
function InsuredAmountForCalculationValueChanged(s, e) {
   InsuredAmountForCalculation_Actions(s, e); 
...
function InsuredAmountForCalculation_Actions(s,e) {
...
function button1921_WebCall(urlForm, formId) {

    var win = window.open(urlForm + '/NNCotizacionVidaResumenPopup.aspx?fromid='+ formId, 'button1921', 'scrollbars=no,resizable=yes,toolbar=no,location=no,directories=no,status=yes,menubar=no,copyhistory=no,width=1000,height=1000,left=100,top=100');
    $('body').append(win);
    $(win).submit();
}
