...
function ClienteProductorValidation(s, e) {
   ClienteProductorBigDataSet(s, e); 
}
...
function ClienteProductorBigDataSet(s, e) { 
...
function ClienteProductorEndCallbackDependency(s, e) { 
    s.SetEnabled(true);
    if (s.GetText() == msgControlsDependencyResource) 
        s.SetText(''); 
    if (ClienteProductor_ForSet != ''){ 
        s.SetValue(ClienteProductor_ForSet); 
        ClienteProductor_ForSet = ''; 
    } 
} 
