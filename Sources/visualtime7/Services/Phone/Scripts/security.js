    ...
    if (versionOracle === -1) {
        ...
        html +='<div class="form-group">' +
            ...
            '<div class="col-md-12 text-right">' +
                '<div class="btn-group">' +                   
                    '<input type="button" data-conn="' + connObject.ConnName + '" class="btnTest btn btn-primary" value="Test" />' +
...
var index = {
    Init: function() {
        ...
        $("#EncryptAll").click(function() {
            $.each($(".btnEncrypt"), function () {
                var catalog = $(this).parent().parent().parent().find(".initialCatalog").val();
                var connObj = {
                    connName: $(this).data("conn"),
                    initialCatalog: (typeof(catalog) === 'undefined') ? "" : catalog,
                    dataSource: $(this).parent().parent().parent().find(".dataSource").val(),
                    userId: $(this).parent().parent().parent().find(".userId").val(),
                    pass1: $(this).parent().parent().parent().find(".pass1").val(),
                    pass2: $(this).parent().parent().parent().find(".pass2").val()
                };
    ...
    RenderAfterInit: function() {
        ...
        $(".btnTest").click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "Security.aspx/TestConnection",
                data: JSON.stringify(connObj),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    if (data.d.IsValid) {
                        alert("La connexion  " + connObj.connName + " se valido correctamente!!!");
                    } else {
                        alert("Ocurrio un error :" + data.d.Message);
                    }
